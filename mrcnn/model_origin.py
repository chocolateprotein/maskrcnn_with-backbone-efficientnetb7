...
# class resnest which will be faster

class ResNestNetBase(object):
  def __init__(self,
               pretrained=True,
               model_type='resnest101'):
    if pretrained:
      weights = 'imagenet'
    else:
      weights = None
    if model_type == 'resnest50':
      self.net = resnest50(include_top = False,
                           weights = weights,
                           input_tensor = None,
      ...
      self.net = resnest200(include_top = False,
                            weights = weights,
                            input_tensor = None,
                            input_shape = (224, 224, 3), classes = 1000)
      
    elif model_type == 'resnest269':
      self.net = resnest269(include_top = False,
                            weights = weights,
                            input_tensor = None,
                            input_shape = (224, 224, 3), classes = 1000)
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    # indices = tf.stack([tf.range(probs.shape[0]), class_ids], axis=1) 이전꺼
...
class MaskRCNN():
    ...
    def build(self, mode, config, ):
        ...
        elif self.model_name[0:7].lower() == "resnest":
          print(self.model_name)                             
          if self.pretrained == 'True':
              res_pre = ResNestNetBase(pretrained=True, model_type = self.model_name.lower())
